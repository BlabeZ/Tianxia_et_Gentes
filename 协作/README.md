...
主调度器只可通过 `python3 scripts/workflow.py task merge --id T-020 --generation 1` 合并 `ready_to_merge` 的任务分支。该命令核对真实分支 tip，以显式 `TEG_MERGE_HEAD` 把已验证 SHA 传给 `.githooks/pre-merge-commit`；冲突、hook 或提交失败时自动 `git merge --abort` 恢复。raw Git merge 无法确定受控候选时默认拒绝。统一校验器和 pre-push 再检查 `pending_validation/pending_test` 的 head 是否已经进入 `main`。`task complete` 只有在任务 head 已是 `main` 祖先时才会置为 `done`。历史提前合并只能逐任务、逐 head 绑定用户确认决策登记，禁止全局豁免。
