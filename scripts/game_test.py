...
def build_report(
    *,
    ...
    git_dirty: bool,
    runner_commit: str,
    runner_machine_id: str,
    runner_environment_checked_at: str,
    profile_hash: str,
    rules_hash: str,
    started_at: str,
    ended_at: str,
    ...
    report: dict[str, Any] = {
        ...
        "runner_environment_checked_at": runner_environment_checked_at,
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        f"- runner 机器：{report.get('runner_machine_id') or '未知'}（环境快照 {report.get('runner_environment_checked_at') or '未知'}）",
    ...
    lines.extend(
        [
            ...
            f"- 消费状态：{json.dumps(report.get('consumed_by'), ensure_ascii=False, sort_keys=True) if report.get('consumed_by') else '未消费'}",
            "",
